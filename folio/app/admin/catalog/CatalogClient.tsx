...
interface CatalogClientProps {
  /** All books fetched server-side from the Open Library-enriched data layer */
  books: BookDetail[]
}
...
export default function CatalogClient({ books }: CatalogClientProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return result
  }, [books, query, genreFilter, formatFilter, availFilter, sortField, sortDir, deletedIds])
  ...
  const totalActive = books.length - deletedIds.size
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-brown-light text-sm mt-1">
            {totalActive} items · search, filter, add or edit
          </p>
        </div>
        <button
      ...
      <div className="bg-parchment border border-sand rounded-2xl overflow-hidden">
        ...
        <div className="px-4 py-3 border-t border-sand bg-sand/10 flex items-center justify-between">
          <p className="text-xs text-brown-light">
            Showing {filtered.length} of {totalActive} items
          </p>
